...
export class AnonymousVoting extends Contract {
  ...
  setCandidates(candidate: account_name): void {
    gchain_assert(Action.sender == this.receiver, "only contract owner can set candidates.");
    ...
    if (!this.votes.candidates.includes(candidate)) {
      ...
      gchain_assert(false, "you also set this account as canditate.");
    }
  }
  ...
  vote(canidate: account_name): void {
    ...
    if (this.votes.candidates.includes(canidate)) {
      ...
      if (existing) {
        gchain_assert(candi.who_voted.includes(hashedvoter), "you have voted this candidates.");
        candi.count += 1;
      ...
      gchain_assert(false, "the candidate is not in candicates list.");
    }
  }
}
