...
export class ArrayMap<K, V> implements Serializable {
    ...
    private storeKey (val: K, ds: DataStream): void {
        if (isInteger<K>(val)) {
            ...
            gchain_assert(false, "unsupport key type for ArrayMap.");
        }
    }
    ...
    private storeValue(val: V[], ds: DataStream): void {
        ...
        if (len == 0) {
            ...
            if (isInteger(t0)) {
                ...
                gchain_assert(false, "unsupport value type for ArrayMap.");
            }
        }
    }
    ...
    private readKey(ds: DataStream): void {
        if (isInteger<K>()) {
            ...
            gchain_assert(false, "key type is not support for ArrayMap.");
        }
    }
    ...
    private readValue(ds: DataStream): void {
        ...
        if (isInteger(t0)) {
            ...
            gchain_assert(false, "value type is not support for ArrayMap.");
        }
    }
