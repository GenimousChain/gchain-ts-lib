...
class MasterContract extends Contract {
    ...
    private startGame(from: account_name): void {
        ...
        gchain_assert(!joined, RNAME(from) + " has joined this game yet.");
    ...
    transfer(from: account_name, to: account_name, val: Asset, memo: string): void {
        ...
        // 从this.receiver帐号向其它人转帐
            ...
            // 如果不是向游戏获胜者转帐，将不被允许
            // 这意味着没有人可以从游戏帐号中转移资产
            gchain_assert(reward && player.reward_status == Reward_waiting, RNAME(to) + " is not win the game. Why you get reward??");
            // 更新发奖状态
        ...
        // 其它情况，一律不允许转帐，不接受转入，也不接受转出
            ...
            gchain_assert(false, "Do not accept transfer operation for any other account!");
        }
    }
    ...
    reveal(): void {
        ...
        gchain_assert(cursor.count > 0, "No player joined this game.");
        // 作为测试，随便挑一个发奖
        cursor.first();
