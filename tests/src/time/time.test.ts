...
export　class TimeUtilTest extends Contract {
    ...
    private do_work(time: string, secs: u32, year: u32, month: u32, day: u32, hours: u32, minutes: u32, seconds: u32, tz: string): void {
        ...
        gchain_assert(valid, "time '" + time + "' is invalid.");
        gchain_assert(time == lt.toString(), "convert to utc string failed: " + lt.toString());
        gchain_assert(lt.year == year, time + " : year mismatche.");
        gchain_assert(lt.month == month, time + " : month mismatche.");
        gchain_assert(lt.day == day, time + " : day mismatche.");
        gchain_assert(lt.hours == hours, time + " : hours mismatche.");
        gchain_assert(lt.minutes == minutes, time + " : minutes mismatche.");
        gchain_assert(lt.seconds == seconds, time + " : seconds mismatche.");

        let gt = GmtTime.localTimeToGmtTime(lt);
        gchain_assert(gt.gmtSeconds == secs, time + " convert to epoch seconds failed: " + intToString(gt.gmtSeconds));

        let utcstr = gt.toLocalTime(tz).toString();
        gchain_assert(utcstr == time, time + " convert to utc string failed: " + utcstr);
    }
