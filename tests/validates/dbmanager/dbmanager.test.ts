...
class DBManagerTest extends Contract {
    ...
    get(db: DBManager<Params>): void {
        ...
        gchain_assert(existing, "DBManagerTest p5 must exist.");
        gchain_assert(p5.name == "p5new", "DBManagerTest p5.name must be p5new");

        gchain_assert(db.recordsCount() == 9, "DBManagerTest recordsCount must be 9.");
        db.erase(5);
        gchain_assert(!db.exists(5), "DBManagerTest p5 must be erased.");
        gchain_assert(db.recordsCount() == 8, "DBManagerTest recordsCount must be 8.");
    ...
    enumrate(db: DBManager<Params>): void {
        ...
        gchain_assert(cursor.hasNext(), "DBManagerTest must true for cursor.hasNext()");
