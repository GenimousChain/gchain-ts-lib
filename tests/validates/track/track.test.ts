...
class TestTrack extends Contract {
    ...
    verifyTypes(key: u64, account: account_name, name: string, age: i32, yes: bool, _u64: u64, strs: string[], nums: u64[], car: Car, cars: Car[]): void {
        var aDbManager: DBManager<TypeSet> = new DBManager<TypeSet>(NAME("types"), NAME("types"));
        var types: TypeSet = new TypeSet();
        gchain_assert(aDbManager.exists(key), key.toString() + " not existing.");
        aDbManager.get(key, types);

        gchain_assert(types.account == account, "Test account_name");
        gchain_assert(types.str == name, "Test string");

        gchain_assert(types.smallNum == age, "Test i32");
        gchain_assert(types.exist == yes, "Test bool");
        gchain_assert(types.longNum == _u64, "Test u64");
        gchain_assert(types.strs.join(",") == strs.join(","), "Test string array");
        gchain_assert(types.nums.join(",") == nums.join(","), "Test u64 arry");
        gchain_assert(types.car.toString() == car.toString(), "Test obj");
        gchain_assert(types.cars.toString() == cars.toString(), "Test obj array");
