...
class TestDbSave extends Contract {
    ...
    testGetCar(): Car {
        ...
        gchain_assert(car.name == "red car ferrari", "testGetCar: checking name failed.");
        gchain_assert(car.id == 2025, "testGetCar: checking id failed.");
        return car;
    }
    ...
    saveingDb(id: u64): void {
        ...
        if (!existing) {
            component.year = id;
            component.name = "gchain";
            component.company = "gchain";
    ...
    verifyDataFromTable(id: u64): void {
        ...
        if (existing) {
            ...
            gchain_assert(component.year == id, "verifyDataFromTable: checking year failed.");
            gchain_assert(component.name == "gchain", "verifyDataFromTable: checking name failed.");
            gchain_assert(component.company == "gchain", "verifyDataFromTable: checking company failed.");
            //Log.s("verifyDataFromTable compare name and company successfully.").flush();
            for (let index = 0; index < this.lenOfArr; index++) {
                gchain_assert(component.intArray[index] == index, "verifyDataFromTable: checking intArray failed.");
                gchain_assert(component.strArray[index] == index.toString(), "verifyDataFromTable: checking intArray failed.");
                gchain_assert(component.carArray[index].id == <u64>index, "verifyDataFromTable: checking carArray failed.");
                gchain_assert(component.personAgeMap.has(index), "verifyDataFromTable: checking personAgeMap failed.");
                gchain_assert(component.personNameMap.has(index.toString()), "verifyDataFromTable: checking personNameMap failed.");
            }
        }
    }
    ...
    testGetMap(): void {
        ...
        for (let index = 0; index < this.lenOfArr; index++) {
            let strVal = mapSave.str_str_map.get(index.toString());
            gchain_assert(strVal == index.toString(), "testGetMap: checking str map failed.");
            let intVal = mapSave.int_int_map.get(index);
            gchain_assert(intVal == index, "testGetMap: checking int map failed.");
        }
    }
}
