...
class TestMap extends Contract {
    ...
    public recovryMapCollector(id: u64, key: string, value: string, intVal: i32): void {
        ...
        gchain_assert(existing, "Database data not existing!");

        gchain_assert(mapCollector.str_int_map.get(key) == <u8>intVal, "recovryMapCollector: checking str_int map failed.");
        gchain_assert(mapCollector.str_str_map.get(key) == value, "recovryMapCollector: checking str_str map failed.");
        gchain_assert(mapCollector.int_str_map.get(<u8>intVal) == value, "recovryMapCollector: checking int_str map failed.");
        gchain_assert(mapCollector.int_int_array_map.get(<u8>intVal).join(",") == u8Arr.join(","), "recovryMapCollector: checking int_intArr map failed.");
        gchain_assert(mapCollector.int_string_array_map.get(<u8>intVal).join(",") == strArr.join(","), "recovryMapCollector: checking int_strArr map failed.");
    }
