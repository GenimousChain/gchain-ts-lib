...
class _EventObject implements Serializable {
    ...
    private serializeStringMap(ds: DataStream, mp: Map<string, string>): void {
        ...
        for (let i: i32 = 0; i < keys.length; i++) {
            gchain_assert(keys[i].length <= 127, "events string is longer than 127.");
 ...
 *
 * @example
 * import { EventObject } from "gchain-ts-lib/src/events";
 */
 ...
 *
 * @example
 * import { emit } from "gchain-ts-lib/src/events";
 */
export function emit(evtname: string, obj: _EventObject): i32 {
    gchain_assert(evtname.length <= 64, "length of event name must be less than 64.");
