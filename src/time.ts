...
class TimeZone {
    ...
    parse(timezone: string): boolean {
        ...
        gchain_assert(tzlabel == 0x2b || tzlabel == 0x2d, "timezone of utc string '" + timezone + "' is invalid.");
        this.isEast = (tzlabel == 0x2b);
        let a = timezone.substring(1, timezone.length);
        let tzhm = a.split(":");
        gchain_assert(tzhm.length == 2, "timezone of utc string '" + timezone + "' is invalid.");
 ...
 *
 * @example
 * import { LocalTime } from "gchain-ts-lib/src/time;"
 */
export class LocalTime {
    ...
    private parseUtcString(utcstr: string): string[] {
        ...
        gchain_assert(dt.length == 2, "utc string '" + utcstr + "' is invalid.");

        let ds = dt[0].split("-");
        gchain_assert(ds.length == 3, "date of utc string '" + dt[0] + "' is invalid.");
        ...
        gchain_assert(hms.length == 3, "time of utc string '" + dt[1] + "' is invalid.");
    ...
    fromUtcString(str: string): boolean {
        ...
        gchain_assert(infos.length == 6, "utc string is invalid.");
        let year = I32.parseInt(infos[0]);
        gchain_assert(!isNaN(year), "year of utc string is invalid.");

        let month = I32.parseInt(infos[1]);
        gchain_assert(!isNaN(month), "month of utc string is invalid.");

        let day = I32.parseInt(infos[2]);
        gchain_assert(!isNaN(day), "day of utc string is invalid.");

        let hours = I32.parseInt(infos[3]);
        gchain_assert(!isNaN(hours), "hours of utc string is invalid.");

        let minutes = I32.parseInt(infos[4]);
        gchain_assert(!isNaN(minutes), "minutes of utc string is invalid.");

        let seconds = I32.parseInt(infos[5]);
        gchain_assert(!isNaN(seconds), "seconds of utc string is invalid.");
 ...
 *
 * @example
 * import { GmtTime } from "gchain-ts-lib/src/time;"
 */
export class GmtTime {
    ...
    toLocalTime(timezone: string): LocalTime {
        ...
        gchain_assert(valid, "timezone string '"+ timezone + "' is invalid.");
 ...
 *
 * @example
 * import { TimeUtil } from "gchain-ts-lib/src/time;"
 */
export class TimeUtil {
    ...
    static toLocalTime(utcStr: string): LocalTime {
        ...
        gchain_assert(valid, "utc string '" + utcStr + "' is invalid.");
        return lt;
    }
